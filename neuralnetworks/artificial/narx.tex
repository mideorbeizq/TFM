...
plus a delayed connection from the output to the input layer (a second TDL). 
...
Thus, in the NARX neural network of \figref{narx}, 
...
In what concern training a NARX network the named \emph{Backpropagation Through Time} version of the backpropagation algorithm can be used (read \subsecref{bptt}). The process can be carried out in one out of the two modes shown in \figref{narxtrainingarch} \cite{menezes2008long}: 
\begin{itemize}
